...
function App() {
    ...
    const removeItem = (itemId: number) => {
        ...
        session.cart = session.cart.filter((item) => itemId)
    };
    console.log('Declare-Area: Re-render!!!');

    return (
        <>
            <h1 ref={titleRef} style={{ color: 'white', backgroundColor: 'red' }}>Vite + React</h1>
            ...
            <button onClick={() => titleRef.current?.scrollIntoView({behavior: 'smooth'})}>Go to the Top</button>
        </>
    );
}
